      ...
      1b:	0f 8f cb 00 00 00    	jg     ec <main+0xec>
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      2b:	e8 7a 0d 00 00       	call   daa <open>
...
fork1(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      ...
      if(chdir(buf+3) < 0)
      c9:	e8 0c 0d 00 00       	call   dda <chdir>
  ...
  while((fd = open("console", O_RDWR)) >= 0){
    if(fd >= 3){
      ...
      ef:	50                   	push   %eax
      f0:	e8 9d 0c 00 00       	call   d92 <close>
      break;
...
runcmd(struct cmd *cmd)
{
  ...
  case PIPE:
    ...
    if(pipe(p) < 0)
     ...
     1ca:	50                   	push   %eax
     1cb:	e8 aa 0b 00 00       	call   d7a <pipe>
...
fork1(void)
{
  ...
  if(pid == -1)
    ...
    close(p[0]);
     ...
     20d:	e8 80 0b 00 00       	call   d92 <close>
    close(p[1]);
     212:	58                   	pop    %eax
     213:	ff 75 f4             	pushl  -0xc(%ebp)
     216:	e8 77 0b 00 00       	call   d92 <close>
    wait(0);
...
fork1(void)
{
  ...
  case EXEC:
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     260:	52                   	push   %edx
     261:	50                   	push   %eax
     262:	e8 3b 0b 00 00       	call   da2 <exec>
  ...
  case REDIR:
    ...
    close(rcmd->fd);
     ...
     284:	e8 09 0b 00 00       	call   d92 <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     289:	58                   	pop    %eax
     28a:	5a                   	pop    %edx
     28b:	ff 73 10             	pushl  0x10(%ebx)
     28e:	ff 73 08             	pushl  0x8(%ebx)
     291:	e8 14 0b 00 00       	call   daa <open>
     ...
     2d1:	e8 9a fe ff ff       	call   170 <panic>
{
  ...
  case REDIR:
    ...
    if(open(rcmd->file, rcmd->mode) < 0){
     ...
     2ee:	e8 cd 0b 00 00       	call   ec0 <printf>
      exit(-1);
     2f3:	c7 04 24 ff ff ff ff 	movl   $0xffffffff,(%esp)
  ...
  case PIPE:
    ...
    if(fork1() == 0){
     ...
     30f:	6a 01                	push   $0x1
     311:	e8 7c 0a 00 00       	call   d92 <close>
      dup(p[1]);
     316:	58                   	pop    %eax
     317:	ff 75 f4             	pushl  -0xc(%ebp)
     31a:	e8 c3 0a 00 00       	call   de2 <dup>
      close(p[0]);
     31f:	58                   	pop    %eax
     320:	ff 75 f0             	pushl  -0x10(%ebp)
     323:	e8 6a 0a 00 00       	call   d92 <close>
      close(p[1]);
     328:	58                   	pop    %eax
     329:	ff 75 f4             	pushl  -0xc(%ebp)
     32c:	e8 61 0a 00 00       	call   d92 <close>
      runcmd(pcmd->left);
     331:	58                   	pop    %eax
    ...
    if(fork1() == 0){
     ...
     33d:	6a 00                	push   $0x0
     33f:	e8 4e 0a 00 00       	call   d92 <close>
      dup(p[0]);
     344:	5a                   	pop    %edx
     345:	ff 75 f0             	pushl  -0x10(%ebp)
     348:	e8 95 0a 00 00       	call   de2 <dup>
      close(p[0]);
     34d:	59                   	pop    %ecx
     34e:	ff 75 f0             	pushl  -0x10(%ebp)
     351:	e8 3c 0a 00 00       	call   d92 <close>
      close(p[1]);
     356:	58                   	pop    %eax
     357:	ff 75 f4             	pushl  -0xc(%ebp)
     35a:	e8 33 0a 00 00       	call   d92 <close>
      runcmd(pcmd->right);
     35f:	58                   	pop    %eax
...
gets(char *buf, int max)
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     c45:	57                   	push   %edi
     c46:	6a 00                	push   $0x0
     c48:	e8 35 01 00 00       	call   d82 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     cad:	e8 f8 00 00 00       	call   daa <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     cc1:	50                   	push   %eax
     cc2:	e8 fb 00 00 00       	call   dc2 <fstat>
     cc7:	89 c6                	mov    %eax,%esi
  close(fd);
     cc9:	89 1c 24             	mov    %ebx,(%esp)
     ccc:	e8 c1 00 00 00       	call   d92 <close>
  return r;
...
00000d7a <pipe>:
SYSCALL(pipe)
     d7a:	b8 04 00 00 00       	mov    $0x4,%eax
     d7f:	cd 40                	int    $0x40
     d81:	c3                   	ret    

00000d82 <read>:
SYSCALL(read)
     d82:	b8 05 00 00 00       	mov    $0x5,%eax
     d87:	cd 40                	int    $0x40
     d89:	c3                   	ret    

00000d8a <write>:
SYSCALL(write)
     d8a:	b8 10 00 00 00       	mov    $0x10,%eax
     d8f:	cd 40                	int    $0x40
     d91:	c3                   	ret    

00000d92 <close>:
SYSCALL(close)
     d92:	b8 15 00 00 00       	mov    $0x15,%eax
     d97:	cd 40                	int    $0x40
     d99:	c3                   	ret    
...
00000da2 <exec>:
SYSCALL(exec)
     da2:	b8 07 00 00 00       	mov    $0x7,%eax
     da7:	cd 40                	int    $0x40
     da9:	c3                   	ret    

00000daa <open>:
SYSCALL(open)
     daa:	b8 0f 00 00 00       	mov    $0xf,%eax
     daf:	cd 40                	int    $0x40
     db1:	c3                   	ret    

00000db2 <mknod>:
SYSCALL(mknod)
     db2:	b8 11 00 00 00       	mov    $0x11,%eax
     db7:	cd 40                	int    $0x40
     db9:	c3                   	ret    

00000dba <unlink>:
SYSCALL(unlink)
     dba:	b8 12 00 00 00       	mov    $0x12,%eax
     dbf:	cd 40                	int    $0x40
     dc1:	c3                   	ret    

00000dc2 <fstat>:
SYSCALL(fstat)
     dc2:	b8 08 00 00 00       	mov    $0x8,%eax
     dc7:	cd 40                	int    $0x40
     dc9:	c3                   	ret    

00000dca <link>:
SYSCALL(link)
     dca:	b8 13 00 00 00       	mov    $0x13,%eax
     dcf:	cd 40                	int    $0x40
     dd1:	c3                   	ret    

00000dd2 <mkdir>:
SYSCALL(mkdir)
     dd2:	b8 14 00 00 00       	mov    $0x14,%eax
     dd7:	cd 40                	int    $0x40
     dd9:	c3                   	ret    

00000dda <chdir>:
SYSCALL(chdir)
     dda:	b8 09 00 00 00       	mov    $0x9,%eax
     ddf:	cd 40                	int    $0x40
     de1:	c3                   	ret    

00000de2 <dup>:
SYSCALL(dup)
     de2:	b8 0a 00 00 00       	mov    $0xa,%eax
     de7:	cd 40                	int    $0x40
     de9:	c3                   	ret    

00000dea <getpid>:
SYSCALL(getpid)
     dea:	b8 0b 00 00 00       	mov    $0xb,%eax
     def:	cd 40                	int    $0x40
     df1:	c3                   	ret    
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     e98:	e8 ed fe ff ff       	call   d8a <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     f08:	50                   	push   %eax
     f09:	57                   	push   %edi
     f0a:	e8 7b fe ff ff       	call   d8a <write>
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ...
     f6a:	50                   	push   %eax
     f6b:	57                   	push   %edi
     f6c:	e8 19 fe ff ff       	call   d8a <write>
     ...
     f81:	e8 04 fe ff ff       	call   d8a <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
  write(fd, &c, 1);
     ffe:	50                   	push   %eax
     fff:	57                   	push   %edi
    1000:	e8 85 fd ff ff       	call   d8a <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    104f:	50                   	push   %eax
    1050:	57                   	push   %edi
    1051:	e8 34 fd ff ff       	call   d8a <write>
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    115e:	53                   	push   %ebx
    115f:	e8 8e fc ff ff       	call   df2 <sbrk>
  if(p == (char*)-1)
